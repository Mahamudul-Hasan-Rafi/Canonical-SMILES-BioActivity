...
NEW_PREFIXES = ("graph_enc.", "graph_proj.", "tok_", "fp_emb.", "fp_cnt.", "nbr_")
...
def make_collate(pad_id, fixed_len=None, fp_tokens=False, graphs=False, pic50=False, nbr_feats=None,
    ...
    def collate(batch):
        ...
        if nbr_feats is not None:   # V6: retrieved training analogues (fingerprint, potency, label, similarity)
            pos = 6 if graphs else 5
            gi = np.stack([b[pos][0] for b in batch])
            out["nb_fp"] = torch.from_numpy(nbr_feats["fp"][gi].astype(np.float32))
...
class V3(nn.Module):
    ...
    def __init__(self, backbone="molformer", modalities=MODALITIES, fusion="xattn", use_type_emb=True,
        ...
        if retrieval:                     # V6: neighbour-anchored delta module
            H = 256
            self.nbr_diff = nn.Sequential(nn.Linear(ecfp_dim, H), nn.GELU(), nn.Dropout(dropout), nn.Linear(H, H))
            self.nbr_feat = nn.Linear(3, H)          # similarity, neighbour potency (z), neighbour label
            self.nbr_q = nn.Linear(D, H)
    ...
    def neighbour_update(self, fused, q_fp, nb_fp, nb_pic, nb_lab, nb_sim):
        """Matched-pair reasoning over k retrieved training analogues: each analogue j gives an anchored
        potency estimate  z_j + delta_j  where delta_j is predicted from the fingerprint difference and the
        query context; attention pools the analogues; the result is added to the fused representation."""
        diff = q_fp.unsqueeze(1).to(nb_fp.dtype) - nb_fp
        zp = (nb_pic - self.pic_mu) / self.pic_sd
        h = self.nbr_norm(F.gelu(self.nbr_diff(diff) + self.nbr_feat(torch.stack([nb_sim, zp, nb_lab], -1))
                                 + self.nbr_q(fused).unsqueeze(1)))
        anch = zp + self.nbr_delta(h).squeeze(-1).float()
    ...
    def forward(self, input_ids, attention_mask, ecfp, maccs, descriptors, return_reg=False, **extra):
        ...
        nbr = None
        if "nb_fp" in extra:
            nbr = {"q_fp": ecfp, "nb_fp": extra["nb_fp"], "nb_pic": extra["nb_pic"], "nb_lab": extra["nb_lab"],
                   "nb_sim": extra["nb_sim"]}
...
VARIANTS = {
    ...
    # ── V6: neighbour-anchored reasoning over retrieved training analogues (training fold only) ──
    "graph_mt_knn":    ("V6-K: V5-MT + context from 5 retrieved training analogues (no delta loss)",
    ...
    "reg_first":       ("V8-R1: regression-first (no class sampler, early stop on val RMSE; pilot-selected)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, task="reg", sampler=False,
                             select_metric="rmse")),
    # Top-5 Optuna trials per branch, confirmed on full CV (the proxy argmax may not transfer)
    "graph_mt_t1": ("Optuna trial 12 (rank 1, proxy 0.9384)",
    ...
    "graph_mt_t2": ("Optuna trial 32 (rank 2, proxy 0.9380)",
                        dict(_V4A, graph=True, new_lr_mult=3.0, aux_pic50=0.23324756606369065,
                             hp_override={"num_heads": 8, "hidden_dim": 256, "dropout": 0.1, "num_classifier_layers": 2, "n_cross_layers": 2, "batch_size": 32, "learning_rate": 2.5882875949074086e-05, "weight_decay": 2.4998778915410575e-05})),
    "graph_mt_t3": ("Optuna trial 27 (rank 3, proxy 0.9371)",
                        dict(_V4A, graph=True, new_lr_mult=3.0, aux_pic50=0.17277624814401973,
                             hp_override={"num_heads": 8, "hidden_dim": 256, "dropout": 0.1, "num_classifier_layers": 2, "n_cross_layers": 2, "batch_size": 32, "learning_rate": 1.939505139268738e-05, "weight_decay": 4.428550565876655e-05})),
    "graph_mt_t4": ("Optuna trial 20 (rank 4, proxy 0.9370)",
                        dict(_V4A, graph=True, new_lr_mult=1.0, aux_pic50=0.1778877623259894,
                             hp_override={"num_heads": 8, "hidden_dim": 256, "dropout": 0.1, "num_classifier_layers": 2, "n_cross_layers": 2, "batch_size": 32, "learning_rate": 1.7809620944011428e-05, "weight_decay": 1.8828084319682743e-05})),
    "graph_mt_t5": ("Optuna trial 35 (rank 5, proxy 0.9366)",
                        dict(_V4A, graph=True, new_lr_mult=3.0, aux_pic50=0.13315601767666627,
                             hp_override={"num_heads": 8, "hidden_dim": 256, "dropout": 0.1, "num_classifier_layers": 2, "n_cross_layers": 2, "batch_size": 32, "learning_rate": 2.747186726390857e-05, "weight_decay": 4.3547195614160796e-05})),
    "reg_delta_t1": ("Optuna trial 30 (rank 1, proxy 0.7395)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, task="reg", sampler=False,
    ...
    "reg_delta_t2": ("Optuna trial 33 (rank 2, proxy 0.7468)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, task="reg", sampler=False,
                             select_metric="rmse", retrieval=5, delta_w=0.15823460178965656,
                             hp_override={"num_heads": 4, "hidden_dim": 256, "dropout": 0.2, "num_classifier_layers": 4, "n_cross_layers": 2, "batch_size": 32, "learning_rate": 1.0558464830866672e-05, "weight_decay": 0.0004924394271080676})),
    "reg_delta_t3": ("Optuna trial 34 (rank 3, proxy 0.7482)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, task="reg", sampler=False,
                             select_metric="rmse", retrieval=5, delta_w=0.16648369560007464,
                             hp_override={"num_heads": 4, "hidden_dim": 512, "dropout": 0.2, "num_classifier_layers": 4, "n_cross_layers": 2, "batch_size": 64, "learning_rate": 1.0803841501546213e-05, "weight_decay": 0.000411219364193031})),
    "reg_delta_t4": ("Optuna trial 39 (rank 4, proxy 0.7489)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, task="reg", sampler=False,
                             select_metric="rmse", retrieval=5, delta_w=0.1510813420688082,
                             hp_override={"num_heads": 4, "hidden_dim": 512, "dropout": 0.2, "num_classifier_layers": 4, "n_cross_layers": 2, "batch_size": 64, "learning_rate": 9.009220812241072e-06, "weight_decay": 0.00030908258638522864})),
    "reg_delta_t5": ("Optuna trial 31 (rank 5, proxy 0.7508)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, task="reg", sampler=False,
                             select_metric="rmse", retrieval=5, delta_w=0.163555132392719,
                             hp_override={"num_heads": 4, "hidden_dim": 256, "dropout": 0.2, "num_classifier_layers": 4, "n_cross_layers": 2, "batch_size": 32, "learning_rate": 2.8937765861458122e-05, "weight_decay": 0.0008480287972527906})),
    ...
    "mt_no_ecfp":        ("V5-MT ablation: - ECFP fingerprint view",
                        dict(_V4A, graph=True, new_lr_mult=10.0, aux_pic50=0.1, modalities=("smiles", "maccs", "desc"))),
    "mt_no_maccs":       ("V5-MT ablation: - MACCS view",
                        dict(_V4A, graph=True, new_lr_mult=10.0, aux_pic50=0.1, modalities=("smiles", "ecfp", "desc"))),
    "mt_no_desc":        ("V5-MT ablation: - descriptor view",
                        dict(_V4A, graph=True, new_lr_mult=10.0, aux_pic50=0.1, modalities=("smiles", "ecfp", "maccs"))),
    "mt_no_graph":       ("V5-MT ablation: - D-MPNN graph view",
                        dict(_V4A, graph=False, new_lr_mult=10.0, aux_pic50=0.1)),
    "mt_concat":         ("V5-MT ablation: - cross-modal Transformer (concat fusion)",
                        dict(_V4A, graph=True, new_lr_mult=10.0, aux_pic50=0.1, fusion="concat", use_type_emb=False, use_gate=False)),
    "mt_no_gate":        ("V5-MT ablation: - per-modality gate",
                        dict(_V4A, graph=True, new_lr_mult=10.0, aux_pic50=0.1, use_gate=False)),
    "mt_no_pic50":       ("V5-MT ablation: - auxiliary potency head (pure classifier)",
                        dict(_V4A, graph=True, new_lr_mult=10.0)),
    "mt_fp1024":         ("V5-MT ablation: binary ECFP 1024 instead of count/chiral 2048",
                        dict(graph=True, new_lr_mult=10.0, aux_pic50=0.1)),
...
def train_one(cfg: RunCfg, feats, tr_idx, vl_idx, eval_sets: dict, seed: int, device,
    ...
    if cfg.retrieval:                                    # V6: analogues retrieved from the TRAINING fold only
        import v4_modules
        S = v4_modules.get_sim_matrix(feats["smiles"])
        tr_arr = np.asarray(tr_idx)

        def nbr_of(idx):
            sub = S[np.ix_(np.asarray(idx), tr_arr)].astype(np.float32)
            same = np.asarray(idx)[:, None] == tr_arr[None, :]
            sub[same] = -1.0                              # never retrieve the molecule itself
            top = np.argsort(-sub, axis=1)[:, :cfg.retrieval]
            return tr_arr[top], np.take_along_axis(sub, top, 1)
        ckw = dict(ckw, nbr_feats={"fp": feats["ecfp2048c"], "pic": feats["pic50"], "lab": feats["labels"]})
    tr_ds = V3Dataset(feats, tr_idx, tokenizer, scaler, augment=augment_ok, nbrs=nbr_of(tr_idx), **dkw)
    vl_ds = V3Dataset(feats, vl_idx, tokenizer, scaler, nbrs=nbr_of(vl_idx), **dkw)
    ev_ds = {k: V3Dataset(feats, idx, tokenizer, scaler, nbrs=nbr_of(idx), **dkw) for k, idx in eval_sets.items()}
